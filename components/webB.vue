<template>
  <div>
    <div v-if="jobseekerData !== null" class="webB">
      <div class="job-main">
        <div class="job-name">
          <div class="job-title" style="color: #0e101a; max-width: 260px">
            {{ jobseekerData.title }}
          </div>
          <div class="job-title" style="color: #15c39c; font-size: 18px">
            {{ jobseekerData.salary }}
          </div>
        </div>
        <div class="base-inf">
          <div class="base-inf-item">
            <img
              style="width: 12px; height: 12px; margin: 0px 6px -3px 0px"
              src="~/assets/img/ic_positioning@2x.png"
              alt="posi"
            />
          ...
          <div class="base-inf-item">
            <img
              style="width: 12px; height: 12px; margin: 0px 6px -3px 0px"
              src="~/assets/img/ic_education@2x.png"
              alt="edu"
            />
          ...
          <div v-if="jobseekerData.workFromHome" class="base-inf-item">
            <img
              style="width: 12px; height: 12px; margin: 0px 6px -3px 0px"
              src="~/assets/img/ic_home@2x.png"
              alt="home"
            />
            <p class="base-inf-text" style="color: #15c39c">
              Can Work From Home
            </p>
          </div>
        </div>
      ...
      <div class="recruiter-inf">
        <div class="recruiter-avatar-container">
          <img
            ...
            v-if="jobseekerData.recruiter.avatar !== ''"
            :src="jobseekerData.recruiter.avatar"
            alt="ava"
          />
          <img
            class="recruiter-avatar"
          ...
          />
          <img
            class="recruiter-avatar-passed"
            src="~/assets/img/ic_verified@2x.png"
            alt="veri"
          />
        </div>
        <div>
          <div class="recruiter-text1">{{ jobseekerData.recruiter.name }}</div>
          <div class="recruiter-text2">
            {{ jobseekerData.recruiter.designation }} ·
            {{ jobseekerData.recruiter.companyName }}
          </div>
        </div>
      </div>
      <el-divider style="margin: 0px" />
        ...
        <div class="company-inf">
          ...
          <div class="company-inf-right">
            ...
            <div class="company-other-inf">
              <span v-if="jobseekerData.company.financing">{{
                jobseekerData.company.financing
              }}</span>
              <span
                v-if="
                  jobseekerData.company.financing &&
                  jobseekerData.company.strength
                "
              >
                ·
              </span>
              <span v-if="jobseekerData.company.strength">{{
                jobseekerData.company.strength
              }}</span>
              <span
                v-if="
                  jobseekerData.company.strength &&
                  jobseekerData.company.industry
                "
              >
                ·
              </span>
              <span v-if="jobseekerData.company.industry">{{
                jobseekerData.company.industry
              }}</span>
            </div>
          </div>
      ...
      <div v-else>
        <div class="job-description">
          <div class="item-title">Job Description</div>
          <div class="work-closed">The position has been closed</div>
        </div>
      </div>
    </div>
    <div v-else>
  ...
  methods: {
    async getCompanyData() {
      const response = await this.$reqGet(
        "/hirect/job-service/web/jobs/" + window.atob(this.$route.query.p)
      );
      ...
      if (response.data.code === 2000) {
        ...
        this.$emit(
          "getRecruiterId",
          this.jobseekerData.recruiter.id,
          this.distinct_id
        );
        this.userInComing();
      } else {
