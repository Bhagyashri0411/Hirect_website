<template>
  <div>
    <div class="get-a-link-to-download">
      ...
      <el-input
        class="get-a-link-input"
        placeholder="Enter 10 digit mobile number"
        @focus="getALinkInputFocus"
        maxlength="10"
        v-model="mobile"
      >
      ...
      <p class="get-a-link-text">
        You’ll receive a link to the Apple App Store or Goolge Play Store. No
        spam.
      </p>
      <el-button class="get-a-link-button" type="primary" @click="sendAppClick"
        >Send app link</el-button
      >
    </div>
    <el-dialog
  ...
  methods: {
    ...
    async sendAppClick() {
      ...
      const data = await this.$reqPost(
        "/hirect/chat-search-service/leads/addBDLead",
        formData
      );
      ...
      const req = await this.$reqPost(
        "/hirect/company-service/postWix",
        vtigerData
      );
