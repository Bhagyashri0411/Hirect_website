    ...
    <div v-if="jobseekerData !== null" class="webC">
      <div class="jobseeker-main">
        <div class="jobseeker-name">
          ...
          <p class="jobseeker-name-inf">
            {{ jobseekerData.designation
            }}<span v-if="jobseekerData.company">
              | {{ jobseekerData.company }}</span
            >
          </p>
        </div>
        <div class="jobseeker-avatar-content">
          <img
            class="jobseeker-avatar"
            v-if="jobseekerData.avatar !== ''"
          ...
          <img
            class="jobseeker-avatar"
            v-else
            src="~/assets/img/male_us_default.png"
            alt="ava"
          />
          <img
            class="jobseeker-avatar-gender"
            v-if="jobseekerData.gender === 'Male'"
            src="~/assets/img/ic_male@2x.png"
            alt="male"
          />
          <img
            class="jobseeker-avatar-gender"
            v-if="jobseekerData.gender === 'Female'"
            src="~/assets/img/ic_female@2x.png"
            alt="female"
          />
        </div>
      </div>
      ...
      <div v-if="seekerStatus === 0">
        <div class="simple-inf-area">
          <div class="base-inf">
            <div class="base-inf-item">
              <img
                style="width: 16px; height: 16px; margin: 0px 6px -3px 0px"
                src="~/assets/img/ic_resume_job@2x.png"
                alt="job"
              />
            ...
            <div class="base-inf-item">
              <img
                style="width: 16px; height: 16px; margin: 0px 6px -3px 0px"
                src="~/assets/img/ic_resume_education@2x.png"
                alt="edu"
              />
            ...
            <div class="base-inf-item">
              <img
                style="width: 16px; height: 16px; margin: 0px 6px -3px 0px"
                src="~/assets/img/ic_resume_birthday@2x.png"
                alt="birth"
              />
          ...
          <div class="skill-tags">
            <div
              class="skill-tags-item"
              v-for="(item, index) in jobseekerData.skillTags"
              :key="index"
            >
              {{ item }}
            </div>
        ...
        <div class="job-preference">
          <div class="item-title">
            <div class="item-title-circle" />
            Job Preference
          </div>
          <div class="job-preference-line">
            <div class="job-preference-line-1" style="font-family: 'Arial'">
              {{ jobseekerData.preferences[0].channel }}
            </div>
            <div class="job-preference-line-1" style="font-family: 'Arial'">
              {{ jobseekerData.preferences[0].salary }}
            </div>
          </div>
          <div class="job-preference-line">
            <div class="job-preference-tags">
              <div
                class="job-preference-item"
                v-for="(item, index) in jobseekerData.preferences[0]
                  .industryTags"
                :key="index"
              >
                {{ item.industry }}
            ...
            <div class="job-preference-line-2">
              {{ jobseekerData.preferences[0].city }}
            </div>
          </div>
        </div>
        <el-divider style="margin: 0px" />
        <div v-if="jobseekerData.experiences.length" class="work-experience">
          ...
          <div
            ...
            v-for="(item, index) in jobseekerData.experiences"
            :key="index"
          >
            <div class="experience-line1">
              <div class="experience-line1-job-name">
                {{ item.designation }} - {{ item.companyName }}
              </div>
              <div class="experience-line1-job-time">
                {{ $dateChange(item.startTime) }} -
                {{ $dateChange(item.endTime) }}
              </div>
            </div>
            <div class="experience-channel">{{ item.channel }}</div>
            <div class="experience-jobcontent">{{ item.jobContent }}</div>
            <div v-if="item.achievement" class="experience-achievement-title">
              Achievement:
            </div>
            <div v-if="item.achievement" class="experience-achievement">
              {{ item.achievement }}
            </div>
          </div>
        </div>
        ...
        <div class="work-experience">
          ...
          <div
            class="work-experience-item"
            v-for="(item, index) in jobseekerData.educations"
            :key="index"
          >
            <div class="experience-line1">
            ...
            <div v-if="item.grade" class="experience-channel">
              Grade: {{ item.grade }}
            </div>
            <div v-if="item.experience" class="experience-jobcontent">
              Experience at school:<br />{{ item.experience }}
            </div>
          </div>
        </div>
        <el-divider style="margin: 0px" />
      ...
      <div v-else>
        <div class="work-closed">The profile has been removed by the user</div>
      </div>
    </div>
    <div v-else>
...
export default {
  ...
  methods: {
    async getJobseekerData() {
      const response = await this.$reqGet(
        "/hirect/candidate-service/web/candidates/" +
          window.atob(this.$route.query.p) +
          "/profile?preferenceId=" +
          window.atob(this.$route.query.r)
      );
