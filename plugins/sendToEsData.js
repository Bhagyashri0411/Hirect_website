...
import { reqPost } from '../plugins/request.js'
...
function getOsInfo() {
  ...
  return { name, version, browser, browserVersion, distinct_id }
}

export async function sendToEsData(eventName, data, uid = '') {
  const { name, version, browser, browserVersion, distinct_id } = getOsInfo()
  const eventData = {
